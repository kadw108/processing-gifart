  ...
  for (int h = floor(wavesHorizon/hSep); h < height + 10; h++) {
    ...
    pg.beginShape();
      ...
      for (int i = 0; i < points.length; i++) {
        float heightPattern = sin((h + 20*frameCount/frameRateConst) / 4);
...
PGraphics fernField(float stalkHeight, color stem, color leaf, int leafType, float fieldHorizon, float verSep, float horSep) {
  ...
  for (int row = 0; row < 2; row++) {
    for (int col = -1; col < (width + horSep)/horSep; col++) {
      ...
      fernFromPoints(pg, randomCurve(stalkHeight, max(2, stalkHeight/200), 180, -90, noise(row, col, 0.2 * frameCount/frameRateConst)), 0, 0, 3, leafType, stem, leaf);
      pg.popMatrix();
    }
  }
  pg.endDraw();
...
void expandingShape() {
    ...
    if (a > 0 && a < 440) {
      waveShape(origin.x, origin.y - 210, a*30, a*36, 90, a);
    }
  }
}
