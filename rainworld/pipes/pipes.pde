...
void setup(){
  ...
  xCircle = loadImage("/home/account/Videos/psychedelic_sky_islands/assets/xCircle3.png");
  
  karma = loadKarma();
  pipes1 = loadImage("Untitled-Artwork-2.png");
  pipes1Mask = loadImage("Untitled-Artwork-1.png");
  pipe2 = loadImage("Untitled-Artwork-10.png");
  pipe2Mask = loadImage("Untitled-Artwork-9.png");
  wires1 = loadRange("Untitled-Artwork-", 3, 5, ".png");
  wires2 = loadRange("Untitled-Artwork-", 6, 8, ".png");
  cat = loadRange("Untitled-Artwork-", 12, 20, ".png", 5);
  catMask = load("mask", 5, ".png");
  
  sky1 = loadString("/home/account/Videos/psychedelic_sky_islands/sky/sky5_edit2/", 2000, ".jpg"); 
...
void draw(){
  int fC = (int) ((frameCount/2) % frameLimit);
  int fC2 = (int) ((frameCount/3) % frameLimit);
  int fC3 = frameCount * 4;
  
  background(#fce294);
  
  // showFrame(String[] array, float deg, float x, float y, float h, float w, int shift, int fC)
  PImage bg = loadImage(sky1[(fC3 + 600) % sky1.length]);
  pushMatrix();
  ...
  // posterizeImage(bg, 1);
  filter(POSTERIZE, 2);
  image(bg, 0, 300, 1100, 700);
  // showFrame(sky1, 0, 0, 100, 1100, 700, 600, fC3);
  popMatrix();
  
  /*
  noStroke();
  blendMode(MULTIPLY);
  fill(lerpColor(#fce3f4, #f2e3fc, sin(fC/frameRateConst)), 100);
  for (int i = 0; i < 40; i++) {
    if (abs(frameRate - i*frameRateConst) < frameRateConst*2) {
      columnField(2, 4, (int)(i * frameRateConst));
    }
  }
  */
  
  blendMode(BLEND);
  tint(lerpColor(#f8f8ff, #f7f0ff, sin(fC/frameRateConst)), 255);
  showFrameBounce(wires2, 0, 0, 0, 500, 500, fC2);
  showFrameBounce(wires2, 0, 1, 0, 500, 500, fC2);
  
  imageMode(CORNER);
  tint(lerpColor(#cd79a7, #bf7eaa, sin(fC/frameRateConst)), 255);
  image(pipes1Mask, 0, 0);
  tint(lerpColor(#ea9aa9, #ea98d0, sin(fC/frameRateConst)), 255);
  image(pipes1, 0, 0);
  
  tint(lerpColor(#fce1e9, #fce4e5, sin(fC/frameRateConst)), 255);
  showFrameBounce(wires1, 0, 0, 0, 500, 500, fC2);
  showFrameBounce(wires1, 0, 1, 0, 500, 500, fC2);
  
  imageMode(CORNER);
  tint(lerpColor(#c389c8, #b899be, sin(fC/frameRateConst)), 255);
  image(pipe2Mask, 0, 0);
  tint(lerpColor(#e9b1ed, #f4add7, sin(fC/frameRateConst)), 255);
  image(pipe2, 0, 0);
  ...
  circle(274, 266, 37);
  tint(255, 250);
  showFrameBounce(catMask, 0, 9, 11, 400, 400, fC);
  // tint(#c07ea9, 255);
  tint(#f0aed9, 255);
  showFrameBounce(cat, 0, 9, 11, 400, 400, fC);
  ...
  float fC = frameCount - 70;
  // glitchCloudCube(float x, float y, float z, float randr, float num, float size, float seed, float speed)
  glitchCloudCube(origin.x, origin.y, 0, fC / frameRateConst * 200, 50 * fC/frameRateConst, fC/frameRateConst * 15, frameCount, 0.1);
  */
    
  saveFrame("save/"+frameCount+".jpg");
  print(frameCount, "\n");
}
